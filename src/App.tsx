...
import { ArrowUp, Bot, Network, Rows3, User2 } from 'lucide-react'
...
function App() {
  const { selectedNode, selectedRows, setSelectedNode, toggleSelectedRow, clearSelections } = useWorkspaceStore()
  ...
  return (
    <main className="relative h-screen bg-muted/30 p-4 pr-[452px]">
      <PanelGroup direction="horizontal" className="h-full overflow-hidden rounded-2xl border bg-background">
        <Panel defaultSize={100} minSize={45}>
          <PanelGroup direction="vertical">
            <Panel defaultSize={58} minSize={35}>
              <section className="flex h-full flex-col border-b">
                <header className="flex items-center gap-2 border-b p-3 font-semibold"><Network className="h-4 w-4" /> Graph Panel</header>
            ...
            <Panel defaultSize={42} minSize={28}>
              <section className="flex h-full flex-col">
                <header className="flex items-center justify-between border-b p-3 font-semibold">
                  <p className="flex items-center gap-2"><Rows3 className="h-4 w-4" /> Table Panel</p>
      ...
      <aside className="fixed right-0 top-0 h-screen w-[420px] border-l bg-background">
        <div ref={chatViewportRef} className="h-full space-y-4 overflow-y-auto px-4 py-5 pb-36">
          {referenceChips.length > 0 && (
            <div className="sticky top-0 z-10 -mx-1 flex flex-wrap gap-2 bg-background/90 px-1 py-1 backdrop-blur-sm">
              {referenceChips.map((chip) => <span key={chip} className="rounded-full border bg-muted/40 px-2 py-1 text-xs text-muted-foreground">{chip}</span>)}
            </div>
          )}
          ...
          {messages.map((message) => {
            ...
            return (
              <article key={message.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[92%] rounded-2xl px-3 py-2 ${isUser ? 'bg-primary text-primary-foreground' : 'border bg-muted/30 text-foreground'}`}>
                  <p className="mb-1 flex items-center gap-1 text-[11px] font-semibold uppercase opacity-70">{isUser ? <User2 className="h-3 w-3" /> : <Bot className="h-3 w-3" />} {message.role}</p>
                  <div className="text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: markdownToHtml(message.content) }} />
                </div>
              </article>
            )
          })}
          {isSending && <article className="flex justify-start"><div className="max-w-[88%] rounded-2xl border bg-muted/30 px-3 py-2"><p className="text-sm text-muted-foreground">응답 생성 중...</p></div></article>}
        </div>

        <form className="absolute bottom-0 left-0 right-0 border-t bg-background/95 p-3 backdrop-blur" onSubmit={handleSend}>
          <textarea
            className="h-20 w-full resize-none rounded-2xl border bg-background p-3 pr-14 text-sm focus:outline-none"
